package com.cyberprism.libin.artourist.arhandler;
...
import com.cyberprism.libin.artourist.googleplaces.Place;
import com.cyberprism.libin.artourist.googleplaces.PlacesService;
import com.cyberprism.libin.artourist.modals.LocationModal;
...
public abstract class AbstractArchitectCamActivity extends Activity implements ArchitectViewHolderInterface {
	...
	protected void injectData(final String searchKey, final long radius) {
		if (!isLoading) {
			final Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					if (lastKnownLocaton!=null && !isFinishing()) {
						new LoadLocationsAsync(lastKnownLocaton, searchKey, radius).execute();
	 ...
	 * @param data number of places to load (at max)
	 * @return POI information in JSONArray
	 */
	public static JSONArray getPoiInformation(final Location userLocation, final String data, final ArrayList<Place> places) {
        ...
        final String ATTR_RATING = "rating";

        if(places != null) {
            for (int i = 0; i < places.size(); i++) {
                final HashMap<String, String> poiInformation = new HashMap<String, String>();
                poiInformation.put(ATTR_ID, places.get(i).getId());
                poiInformation.put(ATTR_NAME, places.get(i).getName());
                poiInformation.put(ATTR_DESCRIPTION, places.get(i).getVicinity());
                double[] poiLocationLatLon = getRandomLatLonNearby(userLocation.getLatitude(), userLocation.getLongitude());
                poiInformation.put(ATTR_LATITUDE, places.get(i).getLatitude().toString());
                poiInformation.put(ATTR_LONGITUDE, places.get(i).getLongitude().toString());
                final float UNKNOWN_ALTITUDE = -32768f;  // equals "AR.CONST.UNKNOWN_ALTITUDE" in JavaScript (compare AR.GeoLocation specification)
                // Use "AR.CONST.UNKNOWN_ALTITUDE" to tell ARchitect that altitude of places should be on user level. Be aware to handle altitude properly in locationManager in case you use valid POI altitude value (e.g. pass altitude only if GPS accuracy is <7m).
                poiInformation.put(ATTR_ALTITUDE, String.valueOf(UNKNOWN_ALTITUDE));
				poiInformation.put(ATTR_IMAGE, "noimage.png");
                poiInformation.put(ATTR_IMAGE, "noimage.png");
                poiInformation.put(ATTR_RATING, "0.0");
		...
		if(data != "-1" && data != null) {
			LocationModal[] modals = new Gson().fromJson(data, LocationModal[].class);

			for (int i = 0; i < modals.length; i++) {
				LocationModal modal = modals[i];
				final HashMap<String, String> poiInformation = new HashMap<String, String>();
				poiInformation.put(ATTR_ID, modal.getId());
				poiInformation.put(ATTR_NAME, modal.getName());
				poiInformation.put(ATTR_DESCRIPTION, modal.getSpeciality());
				poiInformation.put(ATTR_LATITUDE, modal.getLatitude() + "");
				poiInformation.put(ATTR_LONGITUDE, modal.getLongitude() + "");
				final float UNKNOWN_ALTITUDE = -32768f;
				poiInformation.put(ATTR_ALTITUDE, String.valueOf(UNKNOWN_ALTITUDE));
				poiInformation.put(ATTR_IMAGE, modal.getImage());
                poiInformation.put(ATTR_RATING, modal.getRating());
	...
	class LoadLocationsAsync extends AsyncTask<String, String, String>{
        ...
        public LoadLocationsAsync(Location userLocation, String searchKey, long radius){
		...
		protected String doInBackground(String... params) {
            ...
            String result;
            if (searchKey.contains("museum"))
                result = WebServiceClient.getlocations();
            else
			    result = null;

            PlacesService placesService = new PlacesService(Globals.GOOGLE_PLACE_KEY);
            places =  placesService.findPlaces(userLocation.getLatitude(), userLocation.getLongitude(), searchKey, radius);
			return result;
		}
		...
		protected void onPostExecute(String result) {
            ...
            poiData = getPoiInformation(lastKnownLocaton, result, places);
